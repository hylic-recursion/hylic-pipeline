    ...
    R: Clone + Send + Sync + 'static,
{
    ...
    {
        self.then_lift(
            <<Self::Base as Stage2Base>::Wrap as WrapShared>::build_map_r_bi::<UN, H, R, RNew, _, _>(fwd, bwd),
        )
    }
    ...
    {
        self.then_lift(
            <<Self::Base as Stage2Base>::Wrap as WrapShared>::build_map_n_bi::<UN, UN2, H, R, _, _>(co, contra),
        )
    }
    ...
    fn explain_describe<FmtFold, Emit>(
        self,
        fmt_ctor: FmtFold,
        emit: Emit,
    ) -> Self::With<
    ...
    {
        self.then_lift(
            <<Self::Base as Stage2Base>::Wrap as WrapShared>::build_explain_describe::<UN, H, R, _, _>(fmt_ctor, emit),
        )
    }
}
