...
fn lift_produces_identity_lifted_pipeline() {
    let p: Stage2Pipeline<SeedPipeline<hylic::domain::Shared, u64, u64, u64, u64>, IdentityLift> =
        basic_pipeline().lift();
    let r = p.run_from_slice(
...
fn lift_preserves_semantics_of_base_run() {
    ...
    // same R.
    let a = basic_pipeline().lift().run_from_slice(
        &dom::exec(funnel::Spec::default(4)),
        &[0u64],
        0u64,
    );
    let b = basic_pipeline().lift().run_from_slice(
        &dom::exec(funnel::Spec::default(4)),
        &[0u64],
        0u64,
    );
...
fn clone_and_branch_before_lifting() {
    ...
    let unlifted = base.clone().lift().run_from_slice(
        &dom::exec(funnel::Spec::default(4)),
        &[0u64],
        0u64,
    );
    let zipped = base.clone().lift().zipmap(|r: &u64| *r).run_from_slice(
        &dom::exec(funnel::Spec::default(4)),
        &[0u64],
        0u64,
    );
    let wrapped = base
        .lift()
        .wrap_init(|n: &u64, o: &dyn Fn(&u64) -> u64| o(n) + 1)
        .run_from_slice(
            &dom::exec(funnel::Spec::default(4)),
            &[0u64],
            0u64,
